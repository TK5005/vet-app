...
public class NewTech {
    ...
    private void createUI(){
        ...
        { 
            ...
            user.setCertification(certNo.getText());
            String[] item = certification.getText().split("\n");
            if(Validate()) {
                if (user.getEmpID() == 0)
                    controller.addNewTech(user, item);
                else
                    controller.updateTech(user, item);
            }else{
                createUI();
            }
        }
    ...
    private void loadData(){
         ...
         certNo.setText(user.getCertification());
         if(certs != null)
             for(Certification cert : certs){
                 certification.append(cert.getCertification()+"\n");
             }
    }
    private JPanel getPanel()
    {
        ...
        JPanel certNoPanel = new JPanel();
        certNoPanel.setLayout(new GridLayout(1,2));
        certNoPanel.setBackground(Color.WHITE);
        JLabel certNoLabel = new JLabel("Technician Cert # :");
        certNo = new JTextField();
        certNoPanel.add(certNoLabel);
        certNoPanel.add(certNo);
        ...
        certification.setToolTipText("One Certification Per Line");
        ...
        centerPanel.add(certNoPanel);
        centerPanel.add(Box.createRigidArea(new Dimension(0,5)));


        centerPanel.add(certPanel);
        centerPanel.add(Box.createRigidArea(new Dimension(0,5)));
    ...
    private boolean Validate(){
        if(fName.getText().isEmpty()){
            ...
            return false;
        }else if(certNo.getText().isEmpty()){
